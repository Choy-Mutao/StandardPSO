...
// Sub-programs
double alea_06(double a, double b);
int alea_integer_06(int a, int b);
double perf_06(int s, int function); // Fitness evaluation
...
init:
    ...
    for (s = 0; s < S; s++)  // Positions and velocities
    {
        ...
        for (d = 0; d < D; d++)
        {
            X[s].x[d] = alea_06(xmin[d], xmax[d]);
            V[s].v[d] = (alea_06(xmin[d], xmax[d]) - X[s].x[d]) / 2; // Non uniform
...
{ // Integer random number in [a b]
    int ir;
    double r;
    r = alea_06(0, 1); ir = (int)(a + r * (b + 1 - a));
    if (ir > b) ir = b;
    return ir;
}
...
//===========================================================
double perf_06(int s, int function)
