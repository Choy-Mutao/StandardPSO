...
struct result PSO(struct param param, struct problem pb)
{
	...
	for (s = 0; s < R.SW.S; s++)
	{
		...
		for (d = 0; d < pb.SS.D; d++)
		{
			R.SW.X[s].x[d] = alea(pb.SS.minInit[d], pb.SS.maxInit[d]); // Initial random Position
		}

		for (d = 0; d < pb.SS.D; d++)
		{
			R.SW.V[s].v[d] = (alea(pb.SS.min[d], pb.SS.max[d]) - R.SW.X[s].x[d]) / 2; // Velocity calculate function
		}
