...
//
//Motivationare their PSO versions
//Quite often some authors say they comp
...
//"best" topology. Hence the random approach used here.
//
//Note that is does not mean that each particle is informed by K ones: the number of particles that informs a given one may be any value between 1 (for each particle informs itself) and S.
//
//About initialisation
//Initial positions are chosen at random inside the search space (which is
//supposed to be a hyperparallelepid, and even often a hypercube), according to an uniform distribution.
//
//This is not the best way, but the one of the original PSO.
//
...
//
//However, again, it is not the best approach. The resulting distribution is not even uniform, as for any method that uses an uniform distribution independently for each component.
//
//The mathematically correct approach needs to use an uniform
//distribution inside a hypersphere. It is not that difficult, and indeed used in some PSO versions, but quite different from the original one.
//
//Some results with the standard values
//
//You may want to recode this program in another language. Also you may want to modify it for your own purposes.
//
//Here are some results on classical test functions to help you to check your code.
//
//Dimension D=30
...
//	errno_t err = fopen_s(&f_run, "f_run.txt", "w");
//	if (err != 0)
//	{
...
//	//---------------------------------------------- PARAMETERS
//	S = 10 + (int)(2 * sqrt(D)); // Calculate the size of a swarm, is important for code performance;
//	if (S > S_max) S = S_max;
//	K = 3; // the number of neighbour that a particle will inform to
//
//	// This is the velocity update equation
//	// V(t+1) = wV(t) + U(0,c)(p(t) - x(t)) + U(0,c)(l(t) - x(t))
//	w = 1 / (2 * log(2)); // the parameter w
//	c = 0.5 + log(2); // the parameter c
//
//	printf("\n Swarm size %i", S);
//	printf("\n Coefficients %f %f \n", w, c);
//
...
//		for (d = 0; d < D; d++)
//		{
//			X[s].x[d] = alea(xmin[d], xmax[d], 0);
//			V[s].v[d] = (alea(xmin[d], xmax[d], 0) - X[s].x[d]) / 2; // Non uniform
...
//		for (d  = 0;  d< D; d++)
//		{
//			V[s].v[d] = w * V[s].v[d] + alea(0, c, 0) * (P[s].x[d] - X[s].x[d]);
//			V[s].v[d] = V[s].v[d] + alea(0, c, 0) * (P[g].x[d] - X[s].x[d]);
